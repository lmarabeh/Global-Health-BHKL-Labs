...
function addRegionToData(data) {
    ...
    data.forEach(d => {
        d.Region = regionMap[d.Country] || 'Unknown';
    });
    
    // Log unmapped countries
    const unknownCountries = [...new Set(data.filter(d => d.Region === 'Unknown').map(d => d.Country))];
    if (unknownCountries.length > 0) {
        console.warn('Countries without region mapping:', unknownCountries);
    }
